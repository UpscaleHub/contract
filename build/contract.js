...
let Upscale = (_dec = NearBindgen({}), _dec2 = call({}), _dec3 = call({}), _dec4 = view(), _dec(_class = (_class2 = class Upscale {
  constructor() {
    this.users = [];
  }
  set_swap_preference({
    ...
    this.users[accountId].swapPreference = {
      threshold,
      percentage
    };
    log(`Stored preference for ${accountId}`);
  }
  set_learning_info(info) {
    const accountId = predecessorAccountId();
    this.users[accountId] = {
      ...this.users[accountId],
      ...info
    };
    log(`Stored info for ${accountId}`);
  }
  get_info({
    accountId
  }) {
...
}, _applyDecoratedDescriptor(_class2.prototype, "set_swap_preference", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "set_swap_preference"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "set_learning_info", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "set_learning_info"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_info", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "get_info"), _class2.prototype), _class2)) || _class);
function get_info() {
  const _state = Upscale._getState();
  if (!_state && Upscale._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = Upscale._create();
  if (_state) {
    Upscale._reconstruct(_contract, _state);
  }
  const _args = Upscale._getArgs();
  const _result = _contract.get_info(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Upscale._serialize(_result, true));
}
function set_learning_info() {
  const _state = Upscale._getState();
  if (!_state && Upscale._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = Upscale._getArgs();
  const _result = _contract.set_learning_info(_args);
  Upscale._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Upscale._serialize(_result, true));
}
function set_swap_preference() {
  const _state = Upscale._getState();
  if (!_state && Upscale._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = Upscale._getArgs();
  const _result = _contract.set_swap_preference(_args);
  Upscale._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(Upscale._serialize(_result, true));
}

export { get_info, set_learning_info, set_swap_preference };
